...
class ImportInBackground(BackgroundTaskThread):
    ...
    def adjust_addr(self, sections, addr):
        ...
        if section_name is None:
            log_warn('Section not found in analysis data: {:08x}'.format(addr))
            return None

        # Retrieve section start in BN
        bn_section = self.bv.get_section_by_name(section_name)
        if bn_section is None:
            log_warn('Section not found in BN database: name: {} addr:{:08x}'.
                     format(section_name, addr))
            return None
        ...
        for struct_name, struct_info in structs.items():
            ...
            for member_name, member_info in struct_info['members'].items():
                try:
                    ...
                    log_warn('Failed to apply type ({}) to member ({}): ({})'.
    ...
    def run(self):
        log_info('Importing analysis data from {}'.format(
            self.options.json_file))
        json_array = self.open_json_file(self.options.json_file)
        if self.bv.platform is None:
            log_error('Platform has not been set, cannot import analysis data')
            return
        ...
        log_info('Done importing analysis data')
